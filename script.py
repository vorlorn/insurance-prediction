...
# result without offset optimization
preds_out = pd.DataFrame({"Id": test['Id'].values, "Response": np.round(np.clip(test_preds, 1, 8)).astype(int)})
preds_out = preds_out.set_index('Id')
preds_out.to_csv('xgb_submission.csv')

# train offsets
# Initial guess (only 3,4,5,6 matters: 1,2,3 -> 1, 8->8)
